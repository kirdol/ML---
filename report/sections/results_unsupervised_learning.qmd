...
Also, looking at the biplot, there seems to be 3 distinct "clusters" in the observations. Thus, we will proceed in a cluster analysis.
...
Finally, we can find in the Annex part our tentative for silhouette plots as well as the 3D biplot with 6 clusters instead of 3.

To sum the unsupervised learning part, we can clearly say that there seems to be some factors that are linked together and some observations that can be linked into 3 clusters. These clusters being "Hybrid Vehicles", "Vehicle Characteristics" and "Normal Gas Vehicles" makes sense, as their observations are similar. As for the features, we need 7 dimensions to explain at least 80% of the total variance, meaning that not all the features are concretely linked together and the links are "moderately strong".