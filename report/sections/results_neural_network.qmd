...
history_no_dropout = model_no_dropout.fit(X_train, y_train, epochs=5, batch_size=32, validation_split=0.2, verbose=0)
...
# Train the model with class weights
history_no_dropout = model_no_dropout2.fit(X_train, y_train, epochs=5, batch_size=32, validation_split=0.2, class_weight=class_weights_dict, verbose=0)
...
for rate in dropout_rates:
    ...
    history = model.fit(X_train_split, y_train_split, epochs=5, batch_size=32, class_weight=class_weights_dict, verbose=0)
...
# Train the model and collect training accuracy
history = model.fit(X_train_split,y_train_split, epochs=5, batch_size=32, class_weight=class_weights_dict, validation_data=(X_val, y_val), verbose=0)
...
training_accuracies = []
validation_accuracies = []
test_accuracies = []  # Initialize the test_accuracies list
...
for train_index, val_index in kfold.split(X_train):
    ...
    modelend = create_model(dropout_rate=dropout_rate_to_plot)
    history = modelend.fit(X_train_split, y_train_split, epochs=20, batch_size=32,
    ...
    test_loss, test_accuracy = model.evaluate(X_test, y_test, verbose=0)
    test_accuracies.append(test_accuracy)  # Append the test accuracy for each fold

    # Debugging output
    print(f"Fold results: Training accuracy: {training_accuracy}, Validation accuracy: {validation_accuracy}, Test accuracy: {test_accuracy}")

# Calculate average accuracies
average_training_accuracy = np.mean(training_accuracies)
average_validation_accuracy = np.mean(validation_accuracies)
average_test_accuracy = np.mean(test_accuracies)  # Now this will work correctly
