...
The isse of unbalanced classes, as explained previously can highly weaken the model ability to generalize to new data. The model, will automatically prefer to predict the most frequent classes. We can see in the boxplots below the distribution of the sensitivity and specificity for the classes. Even though, we already dealt in part with the unbalanced class during the cleaning process, as seen in the plot in section [EDA -@sec-make_n_plot], there's still big differences between the classes.
...
Our final accuracy of our model are not as great as we had with our first model but the model that we are using is at least better at representing the data and generalizing to new data. We aslo computed the Cohen's Kappa score which is a good indicator of the model's performance. And as we can see, the model performs well.
