...
As we can see, the tree resulting from our training is much complex, takes a lot of time to train and is not interpretable at all. Also, we can see that the accuracy of the training set is higher than both the observed accuracy and cohen's kappa of the test set, attesting that our model is overfitting. Therefore, let's see the the assessment of the model performance with the K-fold method.

### Training the Decision Tree with 10 K-fold cross-validation

In order to assess the performance of the model, we also tried to use a K-fold cross-validation with 10 folds. Here are our results.
