...
In this section we are going to perform a classification tree analysis on the dataset. We start first to load the necessary packages and the dataset. We then have prepared the data by encoding categorical variables and splitted it into training and testing sets. We start by showing the result of our classification tree without any tuning, then with a K-fold CV and also a pruned classification tree with different max_depth values to find the optimal tree depth that balances between training and test accuracy. Finally, we dealt with class imbalance by doing a Re-sampling. 
...
In order to fight overfitting, we tried to implement a K-fold cross-validation.
...
With a number of fold of 10, we have not been able to fight overfitting, as the training accuracy remaines higher than the test accuracy. 
