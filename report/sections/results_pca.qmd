...
data_processed <- data_cleaned %>%
  ...
  model.matrix(~ . - 1, data = .) %>% # Create dummy variables excluding intercept
  scale() # Standardize the resulting numeric matrix

# Perform PCA
pca_results <- PCA(data_processed, graph = FALSE, scale.unit = TRUE)
