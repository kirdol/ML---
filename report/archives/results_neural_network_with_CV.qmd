...
We will use this dropout rate of 0.2 to train the final model with dropout layers.
...
plt.show()
```
#### Neural Network with Cross-validation and Dropout layers
...
validation_accuracies_CV = []

# Initialize arrays to store accuracies for each epoch
all_training_accuracies = np.zeros((k, num_epochs))
all_validation_accuracies = np.zeros((k, num_epochs))
...
for i, (train_index, val_index) in enumerate(kfold.split(X_train)):
    ...
    model_no_dropout_CV = create_model(X_train.shape[1], y_train.shape[1])
    
    # Train the model
    history_CV = model_no_dropout_CV.fit(X_train_split, y_train_split, epochs=num_epochs, batch_size=32,
                                         class_weight=class_weights_dict, validation_data=(X_val_split, y_val_split), verbose=1)
    
    # Collect accuracy data for each epoch
    all_training_accuracies[i, :] = history_CV.history['accuracy']
    all_validation_accuracies[i, :] = history_CV.history['val_accuracy']
    
    # Debugging output for the last epoch of the current fold
    print(f"Fold {i+1} results: Training accuracy: {history_CV.history['accuracy'][-1]}, Validation accuracy: {history_CV.history['val_accuracy'][-1]}")

# Calculate average accuracies for each epoch
average_training_accuracies = np.mean(all_training_accuracies, axis=0)
average_validation_accuracies = np.mean(all_validation_accuracies, axis=0)

# Plot the accuracies
epochs = range(1, num_epochs + 1)
plt.figure(figsize=(12, 6))
plt.plot(epochs, average_training_accuracies, 'b', label='Average Training Accuracy')
plt.plot(epochs, average_validation_accuracies, 'r', label='Average Validation Accuracy')
plt.title('Training and Validation Accuracy Over 75 Epochs (5 Folds)')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.grid(True)
